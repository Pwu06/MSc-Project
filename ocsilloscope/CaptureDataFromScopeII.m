function waveform = CaptureDataFromScopeII(visaObj)
    fopen(visaObj);
    fprintf(visaObj,':STOP'); %new
    ...
    fprintf(visaObj,':WAV:POINTS 65000'); %16384 cut-off 25MHz
    % Now tell the instrument to digitize channel1
    % fprintf(visaObj,':DIGITIZE CHAN1'); %%% change the channel
    % Wait till complete
    operationComplete = str2double(query(visaObj,'*OPC?'));
    while ~operationComplete
        operationComplete = str2double(query(visaObj,'*OPC?'));
        fprintf(visaObj, ':SYST:ERR?');
    end
    ...
    ylabel('Volts (V)', FontSize=12);
    title('Oscilloscope Data', FontSize=12);
    grid on;
end