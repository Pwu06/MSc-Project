function [response,message_frames,UID,BCC] = digitize_ANTI(Anti,threshold)
    ...
    % can be modified based on the type of the UID size
    ...
    if correct_flag == check_num && BCC_flag == 1
        fprintf("Anticollision Response: %d bits", bits_number)
        fprintf('\n'); 
        disp("Parity Check: Correct")
        disp("BCC    Check: Correct")
        fprintf('Response:       ');
        fprintf('%c ', response);
        fprintf('\n'); 
